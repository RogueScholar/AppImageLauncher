  ...
  for (const auto &pathToAppImage : arguments) {
    ...
    if (hasAlreadyBeenIntegrated(pathToAppImage)) {
      if (desktopFileHasBeenUpdatedSinceLastUpdate(pathToAppImage)) {
        qout() << "AppImage has been integrated already and doesn't need to be "
    ...
    if (QFileInfo(pathToAppImage).absoluteFilePath() !=
      ...
      if (QFile::exists(pathToIntegratedAppImage) &&
        ...
        qerr() << "Could not move AppImage into integration directory (error: "
      ...
      if (!QFile(pathToAppImage).rename(pathToIntegratedAppImage)) {
        qerr() << "Cannot move AppImage to integration directory (permission "
