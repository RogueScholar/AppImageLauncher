...
namespace appimagelauncher { namespace cli { namespace commands {
  void IntegrateCommand::exec(QList<QString> arguments) {
      ...
      for (auto &path : arguments) {
        ...
        // (stuff like TryExec keys etc. being set to paths relative to CWD when
        // running the command , ...)
      ...
      for (const auto &pathToAppImage : arguments) {
        qout() << "Processing " << pathToAppImage << endl;
          ...
          if (! QFileInfo(pathToAppImage).isFile()) {
            qerr() << "Warning: Not a file, skipping: " << pathToAppImage
                   << endl;
            continue;
        }

          if (! isAppImage(pathToAppImage)) {
            qerr() << "Warning: Not an AppImage, skipping: " << pathToAppImage
                   << endl;
            continue;
        }
          ...
          if (hasAlreadyBeenIntegrated(pathToAppImage)) {
              if (desktopFileHasBeenUpdatedSinceLastUpdate(pathToAppImage)) {
                qout()
                    << "AppImage has been integrated already and doesn't need to be "
                       "re-integrated, skipping"
                    << endl;
                continue;
            }

            qout() << "AppImage has already been integrated, but needs to be "
                      "reintegrated"
                   << endl;
        }

        auto pathToIntegratedAppImage =
          ...
          if (QFileInfo(pathToAppImage).absoluteFilePath()
            ...
            qout() << "Moving AppImage to integration directory" << endl;
              ...
              if (QFile::exists(pathToIntegratedAppImage)
                ...
                qerr()
                    << "Could not move AppImage into integration directory (error: "
              ...
              if (! QFile(pathToAppImage).rename(pathToIntegratedAppImage)) {
                qerr()
                    << "Cannot move AppImage to integration directory (permission "
                       "problem?), attempting to copy instead"
                    << endl;
