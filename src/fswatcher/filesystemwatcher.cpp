...
class INotifyEvent {
 ...
 public:
  INotifyEvent(uint32_t mask, QString path):
      mask(mask), path(std::move(path)) {}
};
...
class FileSystemWatcher::PrivateData {
 public:
  enum EVENT_TYPES {
    ...
    // events that indicate a file removal from a directory, e.g., deletion or
 ...
 public:
  ...
  std::vector<INotifyEvent> readEventsFromFd() {
    ...
    const auto error = errno;
      ...
      if (rv == -1) {
        // we're using a non-blocking inotify fd, therefore, if errno is set to
        // EAGAIN, we just didn't find any new events this is not an error case
        if (error == EAGAIN)
          return {};

        throw FileSystemWatcherError(QString("Failed to read from inotify fd: ")
                                     + strerror(error));
    }
    ...
    std::vector<INotifyEvent> events;
      ...
      for (char *p = buffer; p < buffer + rv;) {
        ...
        events.emplace_back(currentEvent->mask,
  ...
  PrivateData(): isRunning(false), watchedDirectories(), mutex(new QMutex) {
    inotifyFd = inotify_init1(IN_NONBLOCK);
      ...
      if (inotifyFd < 0) {
        auto error = errno;
        throw FileSystemWatcherError(
            QString("Failed to initialize inotify, reason: ")
            + strerror(error));
    }
  };
  ...
  bool startWatching(const QDir &directory) {
    ...
    qDebug() << "start watching directory " << directory;
      ...
      if (! directory.exists()) {
        qDebug() << "Warning: directory " << directory.absolutePath()
                 << " does not exist, skipping";
        return true;
    }

    const int watchFd = inotify_add_watch(
        inotifyFd, directory.absolutePath().toStdString().c_str(), mask);

      if (watchFd == -1) {
        const auto error = errno;
        std::cerr << "Failed to start watching: " << strerror(error)
                  << std::endl;
        return false;
    }
  ...
  bool stopWatching(int watchFd) {
    // no matter whether the watch removal succeeds, retrying to remove the
    // watch won't help therefore, we can remove the file descriptor from the
    // map in any case
    ...
    qDebug() << "stop watching watchfd " << watchFd;
      ...
      if (inotify_rm_watch(inotifyFd, watchFd) == -1) {
        const auto error = errno;
        std::cerr << "Failed to stop watching: " << strerror(error)
                  << std::endl;
        return false;
    }
      ...
      while (! watchFdMap.empty()) {
        ...
        const auto watchFd = pair.first;
          ...
          if (! stopWatching(watchFd)) {
            std::cerr << "Warning: Failed to stop watching on file descriptor "
                      << watchFd << std::endl;
        }
      }
  ...
  bool stopWatching(const QDirSet &directories) {
    QMutexLocker lock { mutex };
      ...
      for (const auto &directory : directories) {
        ...
        // reaching the following line means that we couldn't find the requested
        // path in the fd map
        return false;
      }
...
FileSystemWatcher::FileSystemWatcher() {
  ...
  connect(&d->eventsLoopTimer,
          &QTimer::timeout,
          this,
          &FileSystemWatcher::readEvents);
}
...
FileSystemWatcher::FileSystemWatcher(const QDirSet &paths):
    FileSystemWatcher() {
...
bool FileSystemWatcher::startWatching() {
  {
    QMutexLocker lock { d->mutex };
      ...
      if (d->isRunning) {
        qDebug()
            << "tried to start file system watcher while it's running already";
        return true;
    }
  }
...
bool FileSystemWatcher::updateWatchedDirectories(QDirSet watchedDirectories) {
  // the list may contain entries for directories which don't exist already,
  // therefore we have to remove those first so when they'll be created, we'll
  // notice
  {
    // erase-remove doesn't work with sets apparently (see
    // https://stackoverflow.com/a/26833313) therefore we use a simple custom
    // algorithm
  ...
  auto setDifference = [](const QDirSet &toExamine,
                          const QDirSet &toSearchFor) -> QDirSet {
    QDirSet results;

      // QDir behaves weirdly with STL algorithm comparisons etc.
      // therefore we implement this difference algorithm all by ourselves to
  ...
  QDirSet newDirectories =
      setDifference(watchedDirectories, d->watchedDirectories);

  // to stop watching with a fine granularity, we also need to know which
  // directories have been removed
  QDirSet disappearedDirectories =
      setDifference(d->watchedDirectories, watchedDirectories);

  {
    ...
    // if the watching hasn't been started yet, we shouldn't start/stop any
    // watches unfortunately we need an extra variable to track this...
    if (! d->isRunning)
      return true;
  }

  // we must run both stop and start methods, so we cannot directly return false
  // if either fails also, this makes sure the signals are sent even in case
  // either of the following methods fails
  bool rv = true;
