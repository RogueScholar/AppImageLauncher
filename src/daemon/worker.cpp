...
class Worker::PrivateData {
 public:
  ...
  // std::set is unordered, therefore using std::deque to keep the order of the
  // operations
  ...
  class OperationTask: public QRunnable {
   ...
   public:
    OperationTask(const Operation &operation, std::shared_ptr<QMutex> mutex):
        operation(operation), mutex(std::move(mutex)) {}
        ...
        if (type == INTEGRATE) {
          {  // Scope for Output Mutex Locker
            ...
            std::cout << "Integrating: " << path.toStdString() << std::endl;
              ...
              if (! exists) {
                std::cout << "ERROR: file does not exist, cannot integrate"
                          << std::endl;
                return;
            }
            ...
            if (appimage_shall_not_be_integrated(path.toStdString().c_str())) {
              QMutexLocker mutexLocker(mutex.get());
              std::cout << "WARNING: AppImage shall not be integrated, skipping"
                        << std::endl;
              return;
          }
            ...
            if (! installDesktopFileAndIcons(path)) {
              QMutexLocker mutexLocker(mutex.get());
              std::cout << "ERROR: Failed to register AppImage in system"
                        << std::endl;
              return;
          }
 ...
 public:
  ...
  // it starts with the last element, and checks for duplicates until an
  // opposite action is found for instance, when the element shall integrated,
  // it will check for duplicates until an unintegration operation is found
  bool isDuplicate(Operation operation) {
      for (auto it = deferredOperations.rbegin();
           it != deferredOperations.rend();
           ++it) {
          if ((*it).first == operation.first) {
            // if operation type is different, then the operation is new, and
            // should be added to the list if it is equal, it's a duplicate in
            // either case, the loop can be aborted here
...
Worker::Worker() {
  ...
  connect(this,
          &Worker::startTimer,
          this,
          &Worker::startTimerIfNecessary,
          Qt::QueuedConnection);
  connect(&d->deferredOperationsTimer,
          &QTimer::timeout,
          this,
...
void Worker::executeDeferredOperations() {
  ...
  auto outputMutex = std::make_shared<QMutex>();
    ...
    while (! d->deferredOperations.empty()) {
      ...
      QThreadPool::globalInstance()->start(
          new PrivateData::OperationTask(operation, outputMutex));
    }
  ...
  std::cout << "Cleaning up old desktop integration files" << std::endl;
    if (! cleanUpOldDesktopIntegrationResources(true)) {
      std::cout << "Failed to clean up old desktop integration files"
                << std::endl;
  }
  ...
  if (! updateDesktopDatabaseAndIconCaches())
    std::cout << "Failed to update desktop database and icon caches"
              << std::endl;
...
void Worker::scheduleForIntegration(const QString &path) {
  auto operation = std::make_pair(path, INTEGRATE);
    if (! d->isDuplicate(operation)) {
      std::cout << "Scheduling for (re-)integration: " << path.toStdString()
                << std::endl;
...
void Worker::scheduleForUnintegration(const QString &path) {
  auto operation = std::make_pair(path, UNINTEGRATE);
    if (! d->isDuplicate(operation)) {
      std::cout << "Scheduling for unintegration: " << path.toStdString()
                << std::endl;
      d->deferredOperations.push_back(operation);
      emit startTimer();
  }
