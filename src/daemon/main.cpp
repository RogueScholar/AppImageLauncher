...
/**
 * Monitors whether the application binary has changed since the process was
 * started. In such case the application is restarted.
 *
 * @param argv
 */
QTimer *setupBinaryUpdatesMonitor(char *const *argv) {
  ...
  // callback to compare and restart the app if the binary changed since it was
  // started
  QObject::connect(timer, &QTimer::timeout, [argv]() {
    long newBinaryModificationTime = readFileModificationTime(argv[0]);
      if (newBinaryModificationTime != binaryModificationTime) {
        std::cerr
            << "Binary file changed since the applications was started, proceeding to restart it."
            << std::endl;
        execv(argv[0], argv);
    }
  });
...
void initialSearchForAppImages(const QDirSet &dirsToSearch, Worker &worker) {
  // initial search for AppImages; if AppImages are found, they will be
  // integrated, unless they already are
  std::cout << "Searching for existing AppImages" << std::endl;

    for (const auto &dir : dirsToSearch) {
      std::cout << "Searching directory: " << dir.absolutePath().toStdString()
                << std::endl;
        ...
        for (QDirIterator it(dir); it.hasNext();) {
          const auto &path = it.next();
            ...
            if (QFileInfo(path).isFile()) {
              const auto appImageType =
                  appimage_get_type(path.toStdString().c_str(), false);
              const auto isAppImage = 0 < appImageType && appImageType <= 2;

                if (isAppImage) {
                  // at application startup, we don't want to integrate
                  // AppImages that have been integrated already, as that it
                  // slows down very much the integration will be updated as
                  // soon as any of these AppImages is run with AppImageLauncher
                  std::cout << "Found AppImage: " << path.toStdString()
                            << std::endl;

                    if (! appimage_is_registered_in_system(
                            path.toStdString().c_str())) {
                      std::cout << "AppImage is not integrated yet, integrating"
                                << std::endl;
                      worker.scheduleForIntegration(path);
                    } else if (! desktopFileHasBeenUpdatedSinceLastUpdate(
                                   path)) {
                      std::cout
                          << "AppImage has been integrated already but needs to be reintegrated"
                          << std::endl;
                      worker.scheduleForIntegration(path);
                    } else {
                      std::cout << "AppImage integrated already, skipping"
                                << std::endl;
                    }
              }
          }
...
int main(int argc, char *argv[]) {
  ...
  QCommandLineOption listWatchedDirectoriesOption(
    ...
    if (! parser.addOption(listWatchedDirectoriesOption)) {
      throw std::runtime_error(
          "could not add Qt command line option for some reason");
  }
...

  {
    ...
    QCoreApplication::setApplicationVersion(
        QString::fromStdString(version.str()));
  }
  ...
  const auto listWatchedDirectories =
      parser.isSet(listWatchedDirectoriesOption);
  ...
  // it is used to integrate all AppImages initially, and to integrate files
  // found via inotify
  Worker worker;

  // we we update the watched directories, the file system watcher can calculate
  // whether there's new directories to watch these
  QObject::connect(
      &watcher,
      &FileSystemWatcher::newDirectoriesToWatch,
      &app,
      [&worker](const QDirSet &newDirs) {
        if (newDirs.empty()) {
          qDebug() << "No new directories to watch detected";
        } else {
          std::cout
              << "Discovered new directories to watch, integrating existing AppImages initially"
              << std::endl;
  ...
  // whenever a formerly watched directory disappears, we want to clean the menu
  // from entries pointing to AppImages in this directory a good example for
  // this situation is a removable drive that has been unplugged from the
  // computer
  QObject::connect(
      &watcher,
      &FileSystemWatcher::directoriesToWatchDisappeared,
      &app,
      [](const QDirSet &disappearedDirs) {
        if (disappearedDirs.empty()) {
          ...
          std::cout
              << "Directories to watch disappeared, unintegrating AppImages formerly found in there"
              << std::endl;

            if (! cleanUpOldDesktopIntegrationResources(true)) {
              std::cerr
                  << "Error: Failed to clean up old desktop integration resources"
                  << std::endl;
          }
        }
      });
  ...
  // we *have* to do this even though we connect this signal above, as the first
  // update occurs in the constructor and we cannot connect signals before
  // construction has finished for obvious reasons
  ...
  // we regularly want to update
  {
    ...
    QTimer::connect(timer, &QTimer::timeout, &app, [&watcher]() {
      watcher.updateWatchedDirectories(daemonDirectoriesToWatch());
    });
    timer->start();
  }

    // after (re-)integrating all AppImages, clean up old desktop integration
    // resources before start
    if (! cleanUpOldDesktopIntegrationResources()) {
      std::cout << "Failed to clean up old desktop integration resources"
                << std::endl;
  }
  ...
  FileSystemWatcher::connect(&watcher,
                             &FileSystemWatcher::fileChanged,
                             &worker,
                             &Worker::scheduleForIntegration,
                             Qt::QueuedConnection);
  FileSystemWatcher::connect(&watcher,
                             &FileSystemWatcher::fileRemoved,
                             &worker,
                             &Worker::scheduleForUnintegration,
                             Qt::QueuedConnection);
