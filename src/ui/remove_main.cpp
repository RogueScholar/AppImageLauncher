...
int main(int argc, char **argv) {
  ...
  parser.setApplicationDescription(
      QObject::tr("Helper to delete integrated AppImages easily, e.g., from "
                  "the application launcher's context menu"));
  ...
  // the file is only supposed to work on integrated AppImages and _nothing
  // else_
  //    if (!hasAlreadyBeenIntegrated(pathToAppImage)) {
  //        QMessageBox::critical(
  //                nullptr,
  //                QObject::tr("AppImage delete helper error"),
  //                QObject::tr("Refusing to work on non-integrated
  //                AppImage:\n\n%1").arg(pathToAppImage)
  //        );
  //        return 1;
  //    }
