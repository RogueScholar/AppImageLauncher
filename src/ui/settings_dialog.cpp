...
SettingsDialog::SettingsDialog(QWidget *parent)
  ...
  ui->applicationsDirLineEdit->setPlaceholderText(
      integratedAppImagesDestination().absolutePath());
...
// cosmetic changes in lite mode
  ...
  availableFeatures
      << "<br /><br />"
      << tr("<strong>Note: this is an AppImageLauncher Lite build, only "
            "supports a limited set of features</strong><br />"
            "Please install the full version via the provided native packages "
            "to enjoy the full AppImageLauncher experience");
#endif

  ui->featuresLabel->setText(availableFeatures.join('\n'));

  // no matter what tab was selected when saving in Qt designer, we want to
  // start up with the first tab
...
void SettingsDialog::loadSettings() {
  ...
  const auto daemonIsEnabled =
      settingsFile->value("AppImageLauncher/enable_daemon", "true").toBool();
...
void SettingsDialog::saveSettings() {
  ...
  // value it had in the old settings this is supposed to support the option
  // while hiding it in the settings
...
void SettingsDialog::onChooseAppsDirClicked() {
  ...
  // Gtk+ >= 3 segfaults when trying to use the native dialog, therefore we need
...
void SettingsDialog::onAddDirectoryToWatchButtonClicked() {
  ...
  fileDialog.setDirectory(QStandardPaths::locate(
      QStandardPaths::HomeLocation, ".", QStandardPaths::LocateDirectory));
