...
private:
  ...
  // if this is empty, the default value (defined in-code, might change over
  // time) will be chosen the default will not be written to the config file,
  // hence we need a way to detect that state, and it's assumed that when this
  // string is empty, that's the case we could also just compare this directory
  // with the default value just before saving, but IMO it's more obvious to the
  // user that the "default" state is lost after having saved something with the
  // "choose" button in a file dialog
...
private Q_SLOTS:
  ...
  void handleButtonClicked(QAbstractButton *button) {
    if (button ==
        firstRunDialog->buttonBox->button(QDialogButtonBox::RestoreDefaults)) {
      qDebug() << "restore defaults";
      resetDefaults();
    } else if (button ==
               firstRunDialog->buttonBox->button(QDialogButtonBox::Help)) {
      qDebug() << "help";
      QDesktopServices::openUrl(QUrl(
          "https://github.com/TheAssassin/AppImageLauncher/wiki/First-run"));
  ...
  void handleAskMoveCheckBoxStateChange(int state) {
    ...
    for (auto *label : {
             ...
             static_cast<QWidget *>(
                 firstRunDialog->customizeIntegrationDirButton),
         }) {
  ...
  void handleCustomizeIntegrationDirButtonClicked(bool checked = false) {
    ...
    auto newDir = QFileDialog::getExistingDirectory(
        this, tr("Choose integration destination dir"), oldDir);
...
private:
  void updateDestinationDirLabel() {
    ...
    // fallback to default
    if (text.isEmpty())
      text = integratedAppImagesDestination().absolutePath() + " " +
             tr("(default)");
  ...
  void initUi() {
    ...
    // setupUi will modify this dialog so that it looks just like what we
    // designed in Qt Designer
    firstRunDialog->setupUi(this);

    // set up logo in a QLabel
    firstRunDialog->logoLabel->setText("");
    auto pixmap = QPixmap::fromImage(QImage(":/AppImageLauncher.svg"))
    ...
    firstRunDialog->customizeIntegrationDirButton->setIcon(
    ...
    connect(firstRunDialog->buttonBox, &QDialogButtonBox::clicked, this,
            &FirstRunDialog::handleButtonClicked);
    connect(firstRunDialog->askMoveCheckBox, &QCheckBox::stateChanged, this,
            &FirstRunDialog::handleAskMoveCheckBoxStateChange);

    connect(firstRunDialog->customizeIntegrationDirButton,
            &QPushButton::clicked, this,
            &FirstRunDialog::handleCustomizeIntegrationDirButtonClicked);
  }

public:
  FirstRunDialog() { initUi(); }

  void writeConfigFile() {
    bool askToMove =
        firstRunDialog->askMoveCheckBox->checkState() == Qt::Checked;
