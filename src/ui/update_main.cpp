...
int main(int argc, char **argv) {
  ...
  parser.setApplicationDescription(QObject::tr(
      "Updates AppImages after desktop integration, for use by Linux distributions"));

  QApplication app(argc, argv);
  QApplication::setApplicationDisplayName(
      QObject::tr("AppImageLauncher update", "update helper app name"));
      ...
      case 0:
        {
          QMessageBox::information(
              ...
              QObject::tr("Could not find updates for AppImage %1")
                  .arg(pathToAppImage));
          return 0;
        }
      case -1:
      ...
      default:
        {
          QMessageBox::information(
              ...
              QObject::tr("Failed to check for updates:\n\n%1")
                  .arg(updaterStatusMessages.str().c_str()));
          return 1;
        }
    }
  ...
  {
    const auto message =
        QObject::tr("An update has been found for the AppImage %1")
            .arg(pathToAppImage)
    ...
    QCheckBox removeCheckBox(
        QObject::tr("Remove old AppImage after successful update"));
  ...
  // get path to new file, un-integrate old file, remove it, and register
  // updated AppImage
  ...
  if (! updater.pathToNewFile(pathToUpdatedAppImage))
    ...
    if (! QFile::exists(pathToUpdatedAppImage)) {
      criticalUpdaterError(
          QObject::tr("File reported as updated does not exist: %1")
              .arg(pathToUpdatedAppImage));
      return 1;
  }
  ...
  const auto pathToIntegratedAppImage =
    ...
    if (! appimage_shall_not_be_integrated(
            pathToAppImage.toStdString().c_str())) {
        if (! installDesktopFileAndIcons(pathToUpdatedAppImage)) {
          criticalUpdaterError(
              QObject::tr("Failed to register updated AppImage in system"));
          return 1;
      }
  }

  // a crappy attempt to prevent deletion of the updated AppImage in a rare case
  // (see below)
  const auto pathToIntegratedUpdatedAppImage =
      buildPathToIntegratedAppImage(pathToUpdatedAppImage);

    if (removeAfterUpdate) {
        // make sure not to delete the updated(!) AppImage if the filenames of
        // the new and old file are equal in this case, a warning is shown,
        // asking the user whether to overwrite the old file, and in that case
        // we don't need to unregister nor delete the file
        if (pathToIntegratedAppImage != pathToIntegratedUpdatedAppImage) {
            if (appimage_unregister_in_system(
                ...
                != 0) {
              criticalUpdaterError(
                  QObject::tr("Failed to unregister old AppImage in system"));
              return 1;
          }

            if (! QFile::remove(pathToAppImage)) {
              criticalUpdaterError(
                  QObject::tr("Failed to remove old AppImage"));
              return 1;
          }
      }
  }
