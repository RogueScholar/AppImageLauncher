...
int runAppImage(const QString &pathToAppImage, unsigned long argc,
  ...
  std::string pathToBinfmtBypassLauncher =
      privateLibDirPath("binfmt-bypass").toStdString() + "/binfmt-bypass";

  // create new args array for exec()d process
  std::vector<char *> args;

  // first argument is the path to our launcher
  auto pathToBinfmtBypassLauncherBuffer =
      makeVectorBuffer(pathToBinfmtBypassLauncher);
  args.push_back(pathToBinfmtBypassLauncherBuffer.data());

  // first argument is consumed by the bypass launcher
  // the reason we launch the bypass launcher as a new process to save RAM (we
  // have to launch the actual AppImage as a subprocess, and the launcher
  // executable has a much lower memory footprint)
...
int main(int argc, char **argv) {
  ...
  auto pathToAppImage = QDir(QString(argv[1])).absolutePath();
    ...
    if (getenv("APPIMAGELAUNCHER_DISABLE") != nullptr
      ...
      return runAppImage(
          pathToAppImage, appImageArgv.size(), appImageArgv.data());
  }
  ...
  auto config = getConfig();
    ...
    // if the user opted out of the "ask move" thing, we can just run the
    // AppImage
  ...
  auto integrateAndRunAppImage =
      [&pathToAppImage, &pathToIntegratedAppImage, &appImageArgv]() {
        ...
        if (! updateDesktopDatabaseAndIconCaches())
          ...
          if (rv == INTEGRATION_FAILED) {
            ...
            return runAppImage(pathToIntegratedAppImage,
                               appImageArgv.size(),
                               appImageArgv.data());
          }
      };

    // after checking whether the AppImage can/must be run without integrating
    // it, we now check whether it actually has been integrated already
    if (hasAlreadyBeenIntegrated(pathToAppImage)) {
      auto updateAndRunAppImage = [&pathToAppImage, &appImageArgv]() {
        // in case there was an update of AppImageLauncher, we should should
        // also update the desktop database and icon caches
        if (! desktopFileHasBeenUpdatedSinceLastUpdate(pathToAppImage)) {
          if (! updateDesktopFileAndIcons(pathToAppImage))
            return 1;

          // make sure the icons in the launcher are refreshed after updating
          // the desktop file
          if (! updateDesktopDatabaseAndIconCaches())
            return 1;
      }
      ...
      {
        auto directoriesNotToAskAboutMovingFor =
            daemonDirectoriesToWatch(config);

        // normally the main integration destination should be contained
        // but bugs happen, and we want to be sure not to create a weird
        // situation where you'd be asked about moving files into yet the
        // directory you want to move them into
        ...
        // not so fast: even if it's not in the main integration directory,
        // there's more viable locations where AppImages may reside just fine
        if (needToAskAboutMoving) {
            for (const auto &additionalLocation :
                 additionalAppImagesLocations()) {
        ...
        if (needToAskAboutMoving) {
          auto *messageBox = new QMessageBox(
              ...
              QMessageBox::tr(
                  "AppImage %1 has already been integrated, but it is "
                  ...
                  QObject::tr(
                      "The directory the integrated AppImages are stored "
          ...
          QApplication::exec();
            ...
            // if the user selects No, then continue as if the AppImage would
            ...
            if (messageBox->clickedButton()
                ...
                if (appimage_unregister_in_system(
                        pathToAppImage.toStdString().c_str(), false)
                    != 0) {
