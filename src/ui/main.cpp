...
int runAppImage(const QString &pathToAppImage, unsigned long argc,
                char **argv) {
  // needs to be converted to std::string to be able to use c_str()
  // when using QString and then .toStdString().c_str(), the std::string
  // instance will be an rvalue, and the pointer returned by c_str() will be
  // invalid
  auto x = pathToAppImage.toStdString();
  auto fullPathToAppImage = QFileInfo(pathToAppImage).absoluteFilePath();

  auto type =
      appimage_get_type(fullPathToAppImage.toStdString().c_str(), false);
  if (type < 1 || type > 3) {
    displayError(QObject::tr("AppImageLauncher does not support type %1 "
                             "AppImages at the moment.")
                     .arg(type));
    return 1;
  }

  // first of all, chmod +x the AppImage registerFile
  // not strictly necessary for AppImageLauncherFS, but if AppImageLauncher is
  // going to be removed, the user will be happy the registerFile is executable
  // already
  if (!makeExecutable(fullPathToAppImage)) {
    displayError(QObject::tr("Could not make AppImage executable: %1")
                     .arg(fullPathToAppImage));
    return 1;
  }

  // make sure to restart service in case AppImageLauncher has been updated
  //    if (!fsDaemonHasBeenRestartedSinceLastUpdate()) {
  //        auto rv = system("systemctl --user restart appimagelauncherfs 2>&1
  //        1>/dev/null"); if (rv != 0) {
  //            displayError(QObject::tr("Failed to register AppImage in
  //            AppImageLauncherFS: error while trying to restart
  //            appimagelauncherfs.service after update")); return 1;
  //        }
  //    }

  // make sure appimagelauncherfs service is running
  auto rv =
      system("systemctl --user enable appimagelauncherfs 2>&1 1>/dev/null");
  rv += system("systemctl --user start appimagelauncherfs 2>&1 1>/dev/null");

  if (rv != 0) {
    displayError(
        QObject::tr("Failed to register AppImage in AppImageLauncherFS: error "
                    "while trying to start appimagelauncherfs.service"));
    return 1;
  }
  ...
  QString pathToFSEndpoint =
      "/run/user/" + QString::number(getuid()) + "/appimagelauncherfs/";

  // resolve path to virtual file in map
  QString pathToVirtualAppImage;

  bool registrationWorked = false, pathResolvingWorked = false,
       openingMapFileWorked = false;

  // try to fetch ID of registered AppImage up to 10 times, with increasing
  // timeouts
  for (useconds_t i = 1; i < 10; i++) {
    ...
    while (std::getline(mapFile, currentLine)) {
      if (currentLine.find(pathToAppImage.toStdString()) != std::string::npos) {
        ...
        pathToVirtualAppImage =
            pathToFSEndpoint + "/" + QString::fromStdString(virtualFileName);
        break;
      }
    }
  ...
  if (pathToVirtualAppImage.isEmpty()) {
    ...
    if (!registrationWorked) {
      errorMessage =
          QObject::tr("Failed to register AppImage in AppImageLauncherFS: "
                      "failed to register AppImage path %1")
              .arg(pathToAppImage);
    } else if (!openingMapFileWorked) {
      errorMessage = QObject::tr("Failed to register AppImage in "
                                 "AppImageLauncherFS: could not open map file");
    } else if (!pathResolvingWorked) {
      errorMessage =
          QObject::tr("Failed to register AppImage in AppImageLauncherFS: "
                      "could not find virtual file for AppImage");
    } else {
      // this should _never_ be reachable
      errorMessage = QObject::tr(
          "Failed to register AppImage in AppImageLauncherFS: unknown failure");
    }
  ...
  std::cerr
      << QObject::tr("execv() failed: %1").arg(strerror(error)).toStdString()
      << std::endl;
  return 1;
}
...
// caution: cannot use <widget>.exec() any more, instead call <widget>.show()
// and use QApplication::exec()
...
int main(int argc, char **argv) {
  // create a suitable application object (either graphical (QApplication) or
  // headless (QCoreApplication)) Use a fake argc value to avoid QApplication
  // from modifying the arguments
  ...
  // clean up trash directory
  {
    TrashBin bin;
    if (!bin.cleanUp()) {
      displayError(QObject::tr("Failed to clean up AppImage trash bin: %1")
                       .arg(bin.path()));
    }
  }

  std::ostringstream usage;
  usage << QObject::tr("Usage: %1 [options] <path>").arg(argv[0]).toStdString()
        << std::endl
        << QObject::tr("Desktop integration helper for AppImages, for use by "
                       "Linux distributions.")
               .toStdString()
        << std::endl
        << std::endl
        ...
        << QObject::tr("Display this help and exit").toStdString() << std::endl
        << "  --appimagelauncher-version  "
        << QObject::tr("Display version and exit").toStdString() << std::endl
        << std::endl
        << QObject::tr("Arguments:").toStdString() << std::endl
        << "  path                        "
        << QObject::tr("Path to AppImage (mandatory)").toStdString()
        << std::endl;

  auto displayVersion = [&app]() {
    std::cerr << "AppImageLauncher " << app->applicationVersion().toStdString()
              << std::endl;
  };
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg.startsWith(prefix)) {
      if (arg == prefix + "help") {
        ...
        std::cerr << QObject::tr("Unknown AppImageLauncher option: %1")
                         .arg(arg)
                         .toStdString()
                  << std::endl;
        return 1;
  ...
  if (!QFile(pathToAppImage).exists()) {
    displayError(QObject::tr("Error: no such file or directory: %1")
                     .arg(pathToAppImage));
    return 1;
  }

  // if the users wishes to disable AppImageLauncher, we just run the AppImage
  // as-ish also we don't ever want to integrate symlinks (see #290 for more
  // information)
  if (getenv("APPIMAGELAUNCHER_DISABLE") != nullptr ||
      QFileInfo(pathToAppImage).isSymLink()) {
    return runAppImage(pathToAppImage, appImageArgv.size(),
                       appImageArgv.data());
  }
  ...
  if (config == nullptr ||
      !config->contains("AppImageLauncher/enable_daemon") ||
  ...
  // as we don't want to offer integration over a headless connection, we just
  // run the AppImage
  if (isHeadless()) {
    return runAppImage(pathToAppImage, appImageArgv.size(),
                       appImageArgv.data());
  ...
  if (config->contains("AppImageLauncher/ask_to_move") &&
      !config->value("AppImageLauncher/ask_to_move").toBool()) {
    return runAppImage(pathToAppImage, appImageArgv.size(),
                       appImageArgv.data());
  }

  // check for X-AppImage-Integrate=false
  auto shallNotBeIntegrated =
      appimage_shall_not_be_integrated(pathToAppImage.toStdString().c_str());
  if (shallNotBeIntegrated < 0)
    std::cerr << "AppImageLauncher error: appimage_shall_not_be_integrated() "
                 "failed (returned "
  ...
  auto isTerminalApp =
      appimage_is_terminal_app(pathToAppImage.toStdString().c_str());
  if (isTerminalApp < 0)
    std::cerr << "AppImageLauncher error: appimage_is_terminal_app() failed "
                 "(returned "
              << isTerminalApp << ")" << std::endl;
  ...
  const auto pathToIntegratedAppImage =
      buildPathToIntegratedAppImage(pathToAppImage);

  auto integrateAndRunAppImage = [&pathToAppImage, &pathToIntegratedAppImage,
                                  &appImageArgv]() {
    ...
    if (rv == INTEGRATION_FAILED) {
      ...
      return runAppImage(pathToIntegratedAppImage, appImageArgv.size(),
                         appImageArgv.data());
    }
  };

  // after checking whether the AppImage can/must be run without integrating it,
  // we now check whether it actually has been integrated already
  if (hasAlreadyBeenIntegrated(pathToAppImage)) {
    auto updateAndRunAppImage = [&pathToAppImage, &appImageArgv]() {
      // in case there was an update of AppImageLauncher, we should should also
      // update the desktop database and icon caches
      if (!desktopFileHasBeenUpdatedSinceLastUpdate(pathToAppImage)) {
        if (!updateDesktopFileAndIcons(pathToAppImage))
          return 1;

        // make sure the icons in the launcher are refreshed after updating the
        // desktop file
        if (!updateDesktopDatabaseAndIconCaches())
          return 1;
      }
    ...
    // okay, I'll try to prove you wrong
    {
      ...
      // but bugs happen, and we want to be sure not to create a weird situation
      // where you'd be asked about moving files into yet the directory you want
      // to move them into
      directoriesNotToAskAboutMovingFor.insert(
          integratedAppImagesDestination());
    ...
    // not so fast: even if it's not in the main integration directory, there's
    // more viable locations where AppImages may reside just fine
    ...
    if (needToAskAboutMoving) {
      auto *messageBox = new QMessageBox(
          QMessageBox::Warning, QMessageBox::tr("Warning"),
          QMessageBox::tr("AppImage %1 has already been integrated, but it is "
                          "not in the current integration "
                          "destination directory."
                          "\n\n"
                          "Do you want to move it into the new destination?"
                          "\n\n"
                          "Choosing No will run the AppImage once, and leave "
                          "the AppImage in its current "
                          "directory."
                          "\n\n")
                  .arg(pathToAppImage) +
              // translate separately to share string with the other dialog
              QObject::tr("The directory the integrated AppImages are stored "
                          "in is currently set to:\n"
                          "%1")
                  .arg(integratedAppImagesDestination().path()) +
              "\n",
      ...
      // if the user selects No, then continue as if the AppImage would not be
      // in this directory
      if (messageBox->clickedButton() == messageBox->button(QMessageBox::Yes)) {
        // unregister AppImage, move, and re-integrate
        if (appimage_unregister_in_system(pathToAppImage.toStdString().c_str(),
                                          false) != 0) {
          displayError(QMessageBox::tr(
              "Failed to unregister AppImage before re-integrating it"));
          return 1;
        }
  ...
  explanationStrm << QObject::tr("Integrating it will move the AppImage into a "
                                 "predefined location, "
                  ...
                  << std::endl
                  << QObject::tr(
                         "To remove or update the AppImage, please use the "
                         "context menu of the "
                         "application icon in your task bar or launcher.")
                         .toStdString()
                  << std::endl
                  << std::endl
                  << QObject::tr("The directory the integrated AppImages are "
                                 "stored in is currently "
                                 "set to:")
                         .toStdString()
                  << std::endl
                  << integratedAppImagesDestination().path().toStdString()
                  << std::endl;

  auto explanation = explanationStrm.str();

  std::ostringstream messageStrm;
  messageStrm << QObject::tr("%1 has not been integrated into your system.")
                     .arg(pathToAppImage)
                     .toStdString()
              << "\n\n"
  ...
  auto *messageBox =
      new QMessageBox(QMessageBox::Question, QObject::tr("Desktop Integration"),
                      QString::fromStdString(messageStrm.str()));

  auto *okButton = messageBox->addButton(QObject::tr("Integrate and run"),
                                         QMessageBox::AcceptRole);
  auto *runOnceButton =
      messageBox->addButton(QObject::tr("Run once"), QMessageBox::ApplyRole);

  // *whyever* Qt somehow needs a button with "RejectRole" or the X button won't
  // close the current window...
  auto *cancelButton =
      messageBox->addButton(QObject::tr("Cancel"), QMessageBox::RejectRole);
  // ... but it is fine to hide that button after creating it, so it's not
  // displayed
  cancelButton->hide();

  messageBox->setDefaultButton(QMessageBox::Ok);

  // cannot use messageBox.exec(), will produce SEGFAULTS as QCoreApplications
  // can't show message boxes
  messageBox->show();

  // don't need to cast around, exec() is a static method anyway, and
