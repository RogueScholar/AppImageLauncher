...
template <typename T> T bswap(T val) = delete;

template <> uint16_t bswap(uint16_t val) { return bswap_16(val); }

template <> uint32_t bswap(uint32_t val) { return bswap_32(val); }

template <> unsigned long long bswap(unsigned long long val) {
  return bswap_64(val);
}

template <typename EhdrT, typename ValT>
void swap_data_if_necessary(const EhdrT &ehdr, ValT &val) {
  static_assert(std::is_same<Elf64_Ehdr, EhdrT>::value ||
                    std::is_same<Elf32_Ehdr, EhdrT>::value,
...
off_t get_elf_size(std::ifstream &ifs) {
  static_assert(std::is_same<Elf64_Ehdr, EhdrT>::value ||
                    std::is_same<Elf32_Ehdr, EhdrT>::value,
                "must be Elf{32,64}_Ehdr");
  static_assert(std::is_same<Elf64_Shdr, ShdrT>::value ||
                    std::is_same<Elf32_Shdr, ShdrT>::value,
  ...
  off_t last_shdr_offset =
      elf_header.e_shoff + (elf_header.e_shentsize * (elf_header.e_shnum - 1));
   ...
   */
  off_t sht_end =
      elf_header.e_shoff + (elf_header.e_shentsize * elf_header.e_shnum);
...
bool is_32bit_elf(std::ifstream &ifs) {
  ...
  // for the beginning, we just need to read e_ident to determine ELF class
  // (i.e., either 32-bit or 64-bit) that way, we can decide which way to go the
  // easiest way is to just use the ELF API
  Elf64_Ehdr ehdr;
