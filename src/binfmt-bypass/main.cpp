...
bool copy_and_patch_runtime(int fd, const char *const appimage_filename,
                            const ssize_t elf_size) {
...
// if memfd_create is available, we should use it as it has a few important
// advantages over older solutions like shm_open or classic tempfiles

int create_memfd_with_patched_runtime(const char *const appimage_filename,
                                      const ssize_t elf_size) {
  // as we call exec() after fork() to create a child process (the parent keeps
  // it alive, the child doesn't require access anyway), we enable close-on-exec
...
// it requires a few more lines of code (e.g., changing permissions to limit
// access to the created file) also, we can't just

int create_shm_fd_with_patched_runtime(const char *const appimage_filename,
                                       const ssize_t elf_size) {
  // let's hope that mktemp returns a unique filename; if not, shm_open returns
  // an error, thanks to O_EXCL the file exists only for a fraction of a second
  // normally, so the chances are not too bad
  ...
  // shm_open doesn't survive over exec(), so we _have to_ keep this process
  // alive and create a child for the runtime the good news is: we don't have to
  // worry about setting flags to close-on-exec
  ...
  // open file read-only before unlinking the file, this is the fd we return
  // later otherwise we'll end up with ETXTBSY when trying to exec() it
  ...
  // as long as we don't close the fd, it won't go away, but if we do, the OS
...
int main(int argc, char **argv) {
  ...
  if (fork() == 0) {
    ...
    // preload our library
    char *preload_lib_path =
        find_preload_library(is_32bit_elf(appimage_filename));
  ...
  if (WIFSIGNALED(status) != 0) {
    ...
    log_error(
        "unknown error: child didn't exit with signal or regular exit code\n");
