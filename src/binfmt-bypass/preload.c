...
// saw this trick somewhere on the Internet... don't recall where it was, but it
// works well
#ifndef RTLD_NEXT
...
// TODO: write __init() and call that from all functions, loading all required
// symbols, the AppImage path etc. once to improve performance
...
void __init() {
  static bool initialized = false;
    ...
    if (! initialized) {
      ...
      // get rid of $LD_PRELOAD in the first binary which this library is
      // preloaded into (should be the runtime) the easiest way is to wait for
      // one of these functions to be used, then unset it
      unsetenv("LD_PRELOAD");

      // load symbols from libc
      __libc_readlink = (ssize_t(*)(const char *, void *, size_t))dlsym(
          REAL_LIBC, "readlink");
      __libc_realpath =
          (char *(*)(const char *, char *))dlsym(REAL_LIBC, "realpath");
      __libc_open = (int (*)(const char *, int))dlsym(REAL_LIBC, "open");

        if (__libc_readlink == NULL || __libc_realpath == NULL
            || __libc_open == NULL) {
...
char *__abs_appimage_path() {
  ...
  char *appimage_var = getenv(env_var_name);
    ...
    // make path absolute if needed (best effort, it's better to pass an
    // absolute value)
    if (appimage_var[0] != '/') {
...
__attribute__((visibility("default"))) extern ssize_t
...
__attribute__((visibility("default"))) extern char *realpath(const char *name,
                                                             char *resolved) {
  __init();
...
__attribute__((visibility("default"))) extern int open(const char *file,
                                                       int flags, ...) {
  __init();
