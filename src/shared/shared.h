...
static const auto DEFAULT_INTEGRATION_DESTINATION =
    QString(getenv("HOME")) + "/Applications/";
...
// integrates an AppImage using a standard workflow used across all
// AppImageLauncher applications
...
// askToMove and enableDaemon both are bools but represented as int to add some
// sort of "unset" state < 0: unset; 0 = false; > 0 = true destination is a
// string that, when empty, will be interpreted as "use default"
...
// additional directories to monitor for AppImages, and to permit AppImages to
...
// when a file doesn't belong to the current user, this method shows a dialog
// asking whether to relaunch as that user this can be used when e.g., updating
// AppImages owned by root or other users uses pkexec, gksudo, gksu etc.,
// whatever is available the second argument is the question that will be asked
// in the dialog displayed in case a relaunch is necessary
void checkAuthorizationAndShowDialogIfNecessary(const QString &path,
                                                const QString &question);

// searchs for path to private data directory relative to the current binary's
// location returns empty string if the path cannot be found
QString pathToPrivateDataDirectory();

// clean up desktop integration files installed while originally integrating the
// AppImage
bool unregisterAppImage(const QString &pathToAppImage);

// try to load icon with provided name from AppImageLauncher's fallback icons
// directory returns empty QIcon if such an icon cannot be found you can check
// for errors by calling QIcon::isNull()
