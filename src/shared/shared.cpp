...
bool makeExecutable(const QString &path) {
  struct stat fileStat {};
    ...
    if (stat(path.toStdString().c_str(), &fileStat) != 0) {
      std::cerr << "Failed to call stat() on " << path.toStdString()
                << std::endl;
      return false;
  }

    // no action required when file is executable already
    // this could happen in scenarios when an AppImage is in a read-only
    // location
  ...
  file.write("[appimagelauncherd]\n");
    ...
    if (additionalDirsToWatch.empty()) {
      file.write("# additional_directories_to_watch = "
                 "~/otherApplications:/even/more/applications\n");
    } else {
      file.write("additional_directories_to_watch = ");
...
std::shared_ptr<QSettings> getConfig() {
  auto configFilePath = getConfigFilePath();
    ...
    // while in theory it would have been possible to just write the default
    // location to the file, if we'd ever change it again, we'd leave a lot of
    // systems in the old state, and would have to write some complex code to
    // resolve the situation therefore, the file is simply created, but left
    // empty intentionally
    ...
    switch (proc.exitCode()) {
      ...
      default:
        throw std::runtime_error(
            "Headless detection failed: unexpected exit code from xhost");
    }

  return isHeadless;
}

// avoids code duplication, and works for both graphical and non-graphical
// environments
void displayMessageBox(const QString &title, const QString &message,
                       const QMessageBox::Icon icon) {
    if (isHeadless()) {
      std::cerr << title.toStdString() << ": " << message.toStdString()
                << std::endl;
    } else {
      // little complex, can't use QMessageBox::{critical,warning,...} for the
      // same reason as in main()
      auto *mb =
          new QMessageBox(icon, title, message, QMessageBox::Ok, nullptr);
      mb->show();
      QApplication::exec();
    }
}
...
class Mount {
 ...
 public:
  Mount(QString device, QString mountPoint, QString fsType,
        QString mountOptions):
      device(std::move(device)),
      mountPoint(std::move(mountPoint)), fsType(std::move(fsType)),
      mountOptions(std::move(mountOptions)) {}
...
QSet<QString> additionalAppImagesLocations(const bool includeAllMountPoints) {
  ...
  // we don't want to read files from any FUSE mounted filesystems nor from any
  // virtual filesystems to
  static const auto validFilesystems = {
    "ext2", "ext3", "ext4", "ntfs", "vfat"
  };

  static const auto blacklistedMountPointPrefixes = {
    "/var/lib/schroot", "/run/docker", "/boot", "/sys", "/proc", "/snap",
  };
    ...
    if (includeAllMountPoints) {
        for (const auto &mount : listMounts()) {
          ...
          const auto &fsType     = mount.getFsType();
            ...
            // we have to filter out virtual filesystems, i.e., ones which have
            // a "nonsense" device path any device that doesn't start with / is
            // likely virtual, this is the first indicator
            ...
            // we don't want to mount any loop-mounted or bind-mounted or other
            // devices, only... "native" ones therefore we permit only "real"
            // devices listed within /dev
            if (! device.startsWith("/dev/")) {
              continue;
          }

            // there's a few locations which we know we don't want to search for
            // AppImages in either it's a waste of time or otherwise a bad idea,
            // but it will surely save time *not* to search them
            if (std::find_if(blacklistedMountPointPrefixes.begin(),
            ...
            if (std::find(
                    validFilesystems.begin(), validFilesystems.end(), fsType)
                == validFilesystems.end()) {
              continue;
          }

            // sanity check -- can likely be removed in the future
            if (mountPoint.isEmpty()) {
              const auto message = "empty mount point for mount with device "
                                 + device.toStdString();
              throw std::invalid_argument(message);
          }

          // assemble potential applications location; caller needs to check
          // whether the directory exists before setting up e.g., an inotify
          // watch
...
bool shallMonitorMountedFilesystems(std::shared_ptr<QSettings> config) {
  return config != nullptr
      && config->value("appimagelauncherd/monitor_mounted_filesystems", "false")
             .toBool();
}

QDirSet getAdditionalDirectoriesFromConfig(
    const std::shared_ptr<QSettings> &config) {
  // getConfig might've returned a null pointer, therefore we have to check this
  // before proceeding
  if (config == nullptr)
    return {};
  ...
  QDirSet additionalDirs {};
    ...
    for (auto dirPath : configValue.split(":")) {
        // empty values will, for some reason, be interpreted as "use the home
        // directory" as we don't want to accidentally monitor the home
        // directory, we need to skip those values
        if (dirPath.isEmpty()) {
      ...
      qDebug() << "path after tilde expansion:" << dirPath;
        ...
        // non-absolute paths which don't contain a tilde cannot be resolved
        // safely, they likley depend on the cwd therefore, we need to ignore
        // those
        if (! QFileInfo(dirPath).isAbsolute()) {
          std::cerr << "Warning: path " << dirPath.toStdString()
                    << " can not be resolved, skipping" << std::endl;
          continue;
      }
      ...
      const QDir dir(dirPath);
        ...
        if (! dir.exists()) {
          std::cerr << "Warning: could not find directory "
                    << dirPath.toStdString() << ", skipping" << std::endl;
          continue;
      }
...
QDirSet daemonDirectoriesToWatch(const std::shared_ptr<QSettings> &config) {
  ...
  // however, there's likely additional ones to watch, like a system-wide
  // Applications directory
  {
    bool monitorMountedFilesystems =
        config != nullptr && shallMonitorMountedFilesystems(config);

    const auto additionalDirs =
        additionalAppImagesLocations(monitorMountedFilesystems);
  ...
  {
    const auto configProvidedDirectories =
        getAdditionalDirectoriesFromConfig(config);

    std::copy(configProvidedDirectories.begin(),
              configProvidedDirectories.end(),
...
QString buildPathToIntegratedAppImage(const QString &pathToAppImage) {
  ...
  auto fileName = baseName;
    ...
    // must not use completeSuffix() in combination with completeBasename(),
    // otherwise the final filename is composed incorrectly
...
std::map<std::string, std::string>
    findCollisions(const QString &currentNameEntry) {
  std::map<std::string, std::string> collisions {};

  // default locations of desktop files on systems
  const auto directories = { QString("/usr/share/applications/"),
    ...
    for (const auto &directory : directories) {
      QDirIterator iterator(directory, QDirIterator::FollowSymlinks);
        ...
        while (iterator.hasNext()) {
          ...
          std::shared_ptr<GKeyFile> desktopFile(g_key_file_new(),
                                                gKeyFileDeleter);
          std::shared_ptr<GError *> error(nullptr, gErrorDeleter);

          // if the key file parser can't load the file, it's most likely not a
          // valid desktop file, so we just skip this file
          if (! g_key_file_load_from_file(desktopFile.get(),
                                          filename.toStdString().c_str(),
                                          G_KEY_FILE_KEEP_TRANSLATIONS,
                                          error.get()))
            continue;

          auto *nameEntry = g_key_file_get_string(desktopFile.get(),
          ...
          if (nameEntry == nullptr)
            ...
            if (QString(nameEntry).trimmed().startsWith(
                    currentNameEntry.trimmed())) {
...
bool updateDesktopDatabaseAndIconCaches() {
  ...
  const std::map<std::string, std::string> commands = {
    ...
    { "gtk-update-icon-cache-3.0",
      dataLocation.toStdString() + "/icons/hicolor/ -t" },
    ...
    for (const auto &command : commands) {
        ...
        if (system(("which " + command.first + " 2>&1 1>/dev/null").c_str())
            == 0) {
...
std::shared_ptr<char> getOwnBinaryPath() {
  ...
  if (path == nullptr)
    throw std::runtime_error("Could not detect path to own binary; something "
...
QString privateLibDirPath(const QString &srcSubdirName) {
  ...
  QString privateLibDirPath = installPrefixPath + "/" + PRIVATE_LIBDIR;
    ...
    if (! QDir(privateLibDirPath).exists()) {
      // this makes sure that when we're running from a local dev build, we end
      // up in the right directory very important when running this code from
      // the daemon, since it's not in the same directory as the helpers
      privateLibDirPath = ownBinaryDirPath + "/../" + srcSubdirName;
  }

    // if there is no such directory like <prefix>/bin/../lib/... or the binary
    // is not found there, there is a chance the binary is just next to this one
...
bool installDesktopFileAndIcons(const QString &pathToAppImage,
    ...
    if (appimage_register_in_system(pathToAppImage.toStdString().c_str(), false)
        != 0) {
      displayError(
  ...
  auto handleError = [error, desktopFile]() {
    ...
    if (! g_key_file_load_from_file(
            desktopFile.get(), desktopFilePath, flags, error.get())) {
      handleError();
      return false;
  }

  const auto *nameEntry = g_key_file_get_string(desktopFile.get(),
                                                G_KEY_FILE_DESKTOP_GROUP,
                                                G_KEY_FILE_DESKTOP_KEY_NAME,
                                                error.get());
    ...
    if (resolveCollisions) {
      ...
      collisions.erase(collisions.find(desktopFilePath));
        ...
        if (! collisions.empty()) {
          ...
          // increasing number in brackets is appended to the Name in order to
          // keep the number monotonically increasing, we look for the highest
          // number in brackets in the existing entries, add 1 to it, and append
          // it in brackets to the current desktop file's Name entry
          ...
          QRegularExpression regex(R"(^.*\(([0-9]+)\)$)");
            ...
            for (const auto &collision : collisions) {
              ...
              auto match =
                  regex.match(QString::fromStdString(currentNameEntry));
                ...
                if (match.hasMatch()) {
                  ...
                  const int num           = numString.toInt();
                    ...
                    // monotonic counting, i.e., never try to "be smart" by
                    // e.g., filling in the gaps between previous numbers
          ...
          auto newName =
              QString(nameEntry) + " (" + QString::number(currentNumber) + ")";
          g_key_file_set_string(desktopFile.get(),
                                G_KEY_FILE_DESKTOP_GROUP,
                                G_KEY_FILE_DESKTOP_KEY_NAME,
  ...
  QDirIterator i18nDirIterator(TranslationManager::getTranslationDir());
    ...
    while (i18nDirIterator.hasNext()) {
      ...
      if (! QFileInfo(filePath).isFile()
          || ! (fileName.startsWith("desktopfiles.")
                && fileName.endsWith(".json")))
        continue;

      // check whether filename's format is alright, otherwise parsing the
      // locale might try to access a non-existing (or the wrong) member
      ...
      QFile jsonFile(filePath);
        ...
        if (! jsonFile.open(QIODevice::ReadOnly)) {
          displayWarning(
              QMessageBox::tr(
                  "Could not parse desktop file translations:\nCould "
                  "not open file for reading:\n\n%1")
                  .arg(fileName));
      }

      // TODO: need to make sure that this doesn't try to read huge files at
      // once
      ...
      auto jsonDoc = QJsonDocument::fromJson(data, &parseError);
        ...
        if (parseError.error != QJsonParseError::NoError || jsonDoc.isNull()
            || ! jsonDoc.isObject()) {
          displayWarning(QMessageBox::tr("Could not parse desktop file "
                                         "translations:\nInvalid syntax:\n\n%1")
...
// add Remove action
{
  ...
  g_key_file_set_string(
      desktopFile.get(), removeSectionName, "Name", "Delete this AppImage");
  g_key_file_set_string(
      desktopFile.get(), removeSectionName, "Icon", helperIconName);
...
#else
  removeExecPath << getenv("HOME")
                 << "/.local/lib/appimagelauncher-lite/"
                    "appimagelauncher-lite.AppImage remove";
#endif
  ...
  g_key_file_set_string(desktopFile.get(),
                        removeSectionName,
                        "Exec",
                        removeExecPath.str().c_str());

  // install translations
  auto it = QMapIterator<QString, QString>(removeActionNameTranslations);
    while (it.hasNext()) {
      auto entry = it.next();
      g_key_file_set_locale_string(desktopFile.get(),
                                   removeSectionName,
                                   "Name",
                                   entry.key().toStdString().c_str(),
                                   entry.value().toStdString().c_str());
    }
}
...
// add Update action
{
  appimage::update::Updater updater(pathToAppImage.toStdString());
    ...
    if (! updater.updateInformation().empty()) {
      ...
      g_key_file_set_string(
          desktopFile.get(), updateSectionName, "Name", "Update this AppImage");
      g_key_file_set_string(
          desktopFile.get(), updateSectionName, "Icon", helperIconName);
      ...
      updateExecPath << getenv("HOME")
                     << "/.local/lib/appimagelauncher-lite/"
                        "appimagelauncher-lite.AppImage update";
  #endif
...
g_key_file_set_string_list(desktopFile.get(), G_KEY_FILE_DESKTOP_GROUP,
...
// add version key
const auto version =
    QApplication::applicationVersion().replace("version ", "").toStdString();
g_key_file_set_string(desktopFile.get(), G_KEY_FILE_DESKTOP_GROUP,
                      "X-AppImageLauncher-Version", version.c_str());

  // save desktop file to disk
  if (! g_key_file_save_to_file(desktopFile.get(), desktopFilePath,
                                error.get())) {
    handleError();
    return false;
}
...
IntegrationState integrateAppImage(const QString &pathToAppImage,
  ...
  QDir().mkdir(QFileInfo(QFile(pathToIntegratedAppImage)).dir().absolutePath());
    ...
    if (QFileInfo(pathToAppImage).absoluteFilePath()
        ...
        // if it does, the existing AppImage needs to be removed before rename
        // can be called
        if (QFile(pathToIntegratedAppImage).exists()) {
          ...
          message
              ...
              << std::endl
              << std::endl
              << QObject::tr("Do you wish to overwrite the existing AppImage?")
                     .toStdString()
              << std::endl
              << QObject::tr("Choosing No will run the AppImage once, and "
                             "leave the system in its current state.")
                     .toStdString();
          ...
          QApplication::exec();
            ...
            if (messageBox->clickedButton()
                == messageBox->button(QMessageBox::No)) {
...
QString getAppImageDigestMd5(const QString &path) {
  ...
  auto rv = appimage_get_elf_section_offset_and_length(
      path.toStdString().c_str(), ".digest_md5", &offset, &length);
  ...
  // there seem to be some AppImages out there who actually have the required
  // section embedded, but it's empty therefore we make the assumption that a
  // hash value of zeroes is probably incorrect and recalculate in the extremely
  // rare case in which the AppImage's digest would *really* be that value, we'd
  // waste a bit of computation time, but the chances are so low... who cares,
  // right?
  {
    ...
    if (needToCalculateDigest) {
      // calculate digest
      if (! appimage_type2_digest_md5(path.toStdString().c_str(),
                                      buffer.data()))
        return "";
  }
  ...
  directory.setNameFilters(filters);
    ...
    for (auto desktopFilePath : directory.entryList()) {
      ...
      std::shared_ptr<char> execValue(
          g_key_file_get_string(desktopFile.get(),
                                G_KEY_FILE_DESKTOP_GROUP,
        ...
        // if there is no Exec value in the file, the desktop file is apparently
        // broken, therefore we skip the file
      ...
      // TryExec is optional, although recently the desktop integration
      // functions started to force add such keys with a path to the desktop
      // file (before, if it existed, the key was replaced with the AppImage's
      // path) If it exists, we assume its value is the full path to the
      // AppImage, which can be used to check the existence of the AppImage
      QString appImagePath;
        ...
        // FIXME: the split command for the Exec value might not work if there's
        // a space in the filename we really need a parser that understands the
        // desktop file escaping
        if (! QFile(appImagePath).exists()) {
          if (verbose)
            std::cout << "AppImage no longer exists, cleaning up resources: "
                      << appImagePath.toStdString() << std::endl;

          if (verbose)
            std::cout << "Removing desktop file: "
                      << desktopFilePath.toStdString() << std::endl;

          QFile(desktopFilePath).remove();

          // TODO: clean up related resources such as icons or MIME definitions

          auto *iconValue = g_key_file_get_string(desktopFile.get(),
                                                  G_KEY_FILE_DESKTOP_GROUP,
                                                  G_KEY_FILE_DESKTOP_KEY_ICON,
                                                  nullptr);

            if (iconValue != nullptr) {
              const auto dataLocation = QStandardPaths::writableLocation(
                  QStandardPaths::GenericDataLocation);
              const auto iconsPath = QString::fromStdString(
                  dataLocation.toStdString() + "/share/icons/");

                for (QDirIterator it(iconsPath, QDirIterator::Subdirectories);
                     it.hasNext();) {
                  auto path = it.next();

                    if (QFileInfo(path).completeBaseName().startsWith(
                            iconValue)) {
                      QFile::remove(path);
                  }
                }
...
void checkAuthorizationAndShowDialogIfNecessary(const QString &path,
  ...
  const auto fileOwnerUsername = QFileInfo(path).owner();
    ...
    if (ownUid != fileOwnerUid) {
      ...
      const auto relaunch =
          messageBox->clickedButton() == messageBox->button(QMessageBox::Ok);
      ...
      qDebug() << "ok, attempting relaunch with root helper";
        ...
        // pkexec doesn't retain $DISPLAY etc., as per the man page, so we can't
        // run UI programs with it
        for (const auto &rootHelperFilename :
             { /*"pkexec",*/ "gksudo", "gksu" }) {
          const auto rootHelperPath = which(rootHelperFilename);
          qDebug() << "trying root helper " << rootHelperFilename
                   << rootHelperPath;
          ...
          const auto rv =
              execv(strdup(rootHelperPath.toStdString().c_str()), argv.data());

          // if the execution fails, we should signalize this to the user
          // instead of silently failing over to the next tool
          QMessageBox::critical(
              nullptr,
              QMessageBox::tr("Error"),
              QMessageBox::tr(
                  "Failed to run permissions helper, exited with return code %1")
                  .arg(rv));
          exit(1);
        }
      ...
      QMessageBox::critical(
          ...
          QMessageBox::tr("Error"),
          QMessageBox::tr(
              "Could not find suitable permissions helper, aborting"));
      exit(1);
  }
}
...
QString pathToPrivateDataDirectory() {
  ...
  // if this is run from the build tree, we try a path that can only work within
  // the build directory then, we try the expected install location relative to
  // the main binary
  const auto binaryDirPath = QApplication::applicationDirPath();

  // our helper tools are not shipped in usr/bin but
  // usr/lib/<arch>-linux-gnu/appimagelauncher therefore we need to check for
  // the translations directory relative to this directory as well as
  // <arch-linux-gnu> may not be used in the path, we also check for its parent
  // directory
  QString dataDir = binaryDirPath + "/../../share/appimagelauncher/";
    ...
    if (! QDir(dataDir).exists()) {
      std::cerr << "[AppImageLauncher] Warning: "
                << "Path to private data directory could not be found"
                << std::endl;
      return "";
  }
...
bool unregisterAppImage(const QString &pathToAppImage) {
  auto rv = appimage_unregister_in_system(pathToAppImage.toStdString().c_str(),
                                          false);
...
QIcon loadIconWithFallback(const QString &iconName) {
  ...
  // first we check the directory that would be expected with in the build
  // environment
  QDir fallbackIconDirectory =
      QDir(binaryDir + "/../../resources/" + subdirName);

    // if that doesn't work, we check the private data directory, which should
    // work when AppImageLauncher is installed through the packages or in Lite's
    // AppImage
    if (! fallbackIconDirectory.exists()) {
      auto privateDataDir = pathToPrivateDataDirectory();
        ...
        if (privateDataDir.length() > 0 && QDir(privateDataDir).exists()) {
          fallbackIconDirectory =
              QDir(pathToPrivateDataDirectory() + "/" + subdirName);
      }
  }

    // fallback icons aren't critical enough to exit the application if they
    // can't be found after all, the theme icons may work just as well
    if (! fallbackIconDirectory.exists()) {
      std::cerr
          << "[AppImageLauncher] Warning:"
  ...
  const auto iconPath     = fallbackIconDirectory.filePath(iconFilename);
    ...
    if (! QFileInfo(iconPath).isFile()) {
      std::cerr
          << "[AppImageLauncher] Warning: can't find fallback icon for name"
...
void setUpFallbackIconPaths(QWidget *parent) {
  /**
   * Qt 5.12 adds a feature to add fallback paths for icons. This is a very
   * simple way to automatically load custom icons when the icon theme doesn't
   * provide a suitable alternative. However, we need to support a much older Qt
   * version. Therefore we cannot use this very very handy feature. We basically
   * iterate over all buttons which carry an icon and (re)load it, but this time
   * provide a fallback loaded from our private data directory.
   */
  ...
  const auto buttons = parent->findChildren<QAbstractButton *>();
    ...
    for (const auto &button : buttons) {
      ...
      // loading an "empty" (i.e., isNull() returns true) icon as fallback, as
      // returned by loadIconWithFallback(...), works just fine
