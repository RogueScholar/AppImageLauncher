...
std::shared_ptr<QSettings> getConfig() {
  ...
  // resolve the situation therefore, the file is simply created, but left empty
  // intentionally
...
public:
  Mount(QString device, QString mountPoint, QString fsType,
        QString mountOptions)
      : device(std::move(device)), mountPoint(std::move(mountPoint)),
        fsType(std::move(fsType)), mountOptions(std::move(mountOptions)) {}
...
public:
  ...
  const QString &getMountPoint() const { return mountPoint; }

  const QString &getFsType() const { return fsType; }

  const QString &getMountOptions() const { return mountOptions; }
};
...
QSet<QString> additionalAppImagesLocations(const bool includeAllMountPoints) {
  ...
  if (includeAllMountPoints) {
    for (const auto &mount : listMounts()) {
      ...
      // we have to filter out virtual filesystems, i.e., ones which have a
      // "nonsense" device path any device that doesn't start with / is likely
      // virtual, this is the first indicator
      ...
      // devices, only... "native" ones therefore we permit only "real" devices
      // listed within /dev
      ...
      // AppImages in either it's a waste of time or otherwise a bad idea, but
      // it will surely save time *not* to search them
      ...
      if (std::find(validFilesystems.begin(), validFilesystems.end(), fsType) ==
      ...
      if (mountPoint.isEmpty()) {
        const auto message =
            "empty mount point for mount with device " + device.toStdString();
        throw std::invalid_argument(message);
      }

      // assemble potential applications location; caller needs to check whether
      // the directory exists before setting up e.g., an inotify watch
...
QDirSet
getAdditionalDirectoriesFromConfig(const std::shared_ptr<QSettings> &config) {
...
bool installDesktopFileAndIcons(const QString &pathToAppImage,
...

  {
    ...
    while (i18nDirIterator.hasNext()) {
      ...
      if (!QFileInfo(filePath).isFile() ||
          !(fileName.startsWith("desktopfiles.") && fileName.endsWith(".json")))
        continue;
  ...
  // add Remove action
  {
    ...
    g_key_file_set_string(desktopFile.get(), removeSectionName, "Exec",
  ...
  // add Update action
  {
    ...
    if (!updater.updateInformation().empty()) {
      ...
      g_key_file_set_string(desktopFile.get(), updateSectionName, "Exec",
      ...
      while (it.hasNext()) {
        auto entry = it.next();
        g_key_file_set_locale_string(desktopFile.get(), updateSectionName,
                                     "Name", entry.key().toStdString().c_str(),
                                     entry.value().toStdString().c_str());
      }
    }
  }
...
IntegrationState integrateAppImage(const QString &pathToAppImage,
  ...
  if (QFileInfo(pathToAppImage).absoluteFilePath() !=
    ...
    // if it does, the existing AppImage needs to be removed before rename can
    // be called
    ...
    if (!QFile(pathToAppImage).rename(pathToIntegratedAppImage)) {
      auto *messageBox = new QMessageBox(
          QMessageBox::Critical, QObject::tr("Error"),
...
bool cleanUpOldDesktopIntegrationResources(bool verbose) {
  ...
  for (auto desktopFilePath : directory.entryList()) {
    ...
    std::shared_ptr<GKeyFile> desktopFile(
        g_key_file_new(), [](GKeyFile *p) { g_key_file_free(p); });
    ...
    // TryExec is optional, although recently the desktop integration functions
    // started to force add such keys with a path to the desktop file (before,
    // if it existed, the key was replaced with the AppImage's path) If it
    // exists, we assume its value is the full path to the AppImage, which can
    // be used to check the existence of the AppImage
    ...
    // FIXME: the split command for the Exec value might not work if there's a
    // space in the filename we really need a parser that understands the
    ...
    if (!QFile(appImagePath).exists()) {
      ...
      if (verbose)
        std::cout << "Removing desktop file: " << desktopFilePath.toStdString()
                  << std::endl;
...
void checkAuthorizationAndShowDialogIfNecessary(const QString &path,
  ...
  if (ownUid != fileOwnerUid) {
    ...
    auto *messageBox = new QMessageBox(
        QMessageBox::Warning, QMessageBox::tr("Permissions problem"),
        messageBoxText, QMessageBox::Ok | QMessageBox::Abort, nullptr);
    ...
    // pkexec doesn't retain $DISPLAY etc., as per the man page, so we can't run
    // UI programs with it
    for (const auto &rootHelperFilename : {/*"pkexec",*/ "gksudo", "gksu"}) {
      ...
      // if the execution fails, we should signalize this to the user instead of
      // silently failing over to the next tool
      QMessageBox::critical(
          nullptr, QMessageBox::tr("Error"),
          QMessageBox::tr(
...
QIcon loadIconWithFallback(const QString &iconName) {
  ...
  // fallback icons aren't critical enough to exit the application if they can't
  // be found after all, the theme icons may work just as well
  if (!fallbackIconDirectory.exists()) {
    std::cerr
