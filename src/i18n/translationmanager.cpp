...
TranslationManager::TranslationManager(QCoreApplication &app): app(app) {
  ...
  // we're using primarily short names for translations, so we should load these
...
QString TranslationManager::getTranslationDir() {
  ...
  // previously the path to the repo root dir was embedded to allow for finding
  // the compiled translations this lead to irreproducible builds therefore the
  // files are now generated within the build dir, and we guess the path based
